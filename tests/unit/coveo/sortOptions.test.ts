...
describe("SORT_OPTIONS", () => {
  it("offers exactly Relevance / Dex number / Base stat total, in that order", () => {
    // "Name A-Z" (pokemonname) is deliberately excluded — confirmed live
    // against the org to 400 with InvalidSortValueException, since
    // pokemonname doesn't have "Use for sorting" enabled. See the doc
    // comment in src/coveo/sortOptions.ts.
  ...
  it("never offers a sort by pokemonname (confirmed unsortable server-side this session)", () => {
    for (const option of SORT_OPTIONS) {
      if (option.criterion.by === SortBy.Field) {
        expect(option.criterion.field).not.toBe(POKEMON_FIELDS.name);
      }
    }
  });

  it("sorts Dex number ascending and Base stat total descending", () => {
