...
This file adds Claude-specific process rules on top of `AGENTS.md`'s cross-tool facts (commands, repo map, conventions). Don't restate anything from `AGENTS.md`, `docs/handoff/`, `docs/plan101.md`, `docs/archive/EXECUTION-PLAN.md`, or `docs/archive/EXECUTION-PLAN-v2.md` here — link to them. Those files are the source of truth for current state; this file goes stale the moment it duplicates a fact that changes.

## What this is

A take-home technical assessment for a Coveo Forward Deployed Engineer (FDE) role (the "Pokemon Challenge"): a search experience over pokemondb.net built on the Coveo Cloud Platform via Headless. Full requirements: `docs/Pokemon Challenge (Pre-Sales) - 2026.txt`. Current build status and next steps: `docs/handoff/STATE.md` and `docs/handoff/LATEST.md` — read both first each session, before touching the Coveo admin console or auth code. (`docs/handoff/README.md` explains the directory's layout and rotation mechanics.)

## Process rules

- Before ending a session that changed org config, auth, or app-visible behavior, append a new entry to `docs/handoff/LATEST.md` and update `docs/handoff/STATE.md` in place for any fact that changed (Org details, What's done, What's next, Traps, Reference docs), so the next session doesn't re-derive already-known facts. Once `LATEST.md` holds 10 session entries, rotate it per `docs/handoff/README.md`: move its content to a new `docs/handoff/archive/sessions-NNN-NNN.md`, add a row per session to `docs/handoff/INDEX.md`, and reset `LATEST.md`.
