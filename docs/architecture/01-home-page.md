...
graph TD
    ...
    BBT["BrowseByType (static list, no controller)"]
    Home --> SB
    Home --> BBT
```
...
## Browse by type — now a static list, not a live facet

As of `docs/adr/0011-automatic-facet-generation-on-search-page.md`, `BrowseByType.tsx` is a static list: `POKEMON_TYPES` (`src/coveo/typeColors.ts`), the 18 real Pokemon types, hardcoded. This replaced an earlier version that built a real `buildFacet` against an empty query purely to read live type/count pairs. Hardcoding the type *names* isn't fabricated Pokemon data — types are a fixed, closed, real taxonomy, not a per-Pokemon fact being invented — but it does mean this grid no longer shows a live count next to each type (an accepted tradeoff; `/search` itself still shows real, live counts once landed).

Each type renders as a `Chip` (colored via `getTypeColor`, a decorative-only convention — see below), wrapped in a link built by `buildTypeSearchHref(type)` (`src/coveo/browseByTypeUrl.ts`):
