...
- If Passage Retrieval enablement never lands on the trial org (see docs/archive/EXECUTION-PLAN.md's "Unverified" section), `/api/passages` will surface Coveo's 403 to the caller unchanged — sufficient to demonstrate the route was built and gated externally, per the assessment's stated floor for the bonus tier.
