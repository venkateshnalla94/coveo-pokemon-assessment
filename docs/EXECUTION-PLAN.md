...
**Status as of 2026-08-27 (end of day):** Phase 1's doc/CI fixes and all of Phase 2 are done. Phase 1's hosting steps (initial commit, GitHub repo, Vercel deploy) are deliberately parked — not started, pending explicit go-ahead. Phase 3 is complete (Stage A/B/C all done and verified — see that section). Phase 4 is well underway: real credentials are wired up, the app is genuinely returning live, correctly-faceted, image-populated Pokemon results end to end (verified via the e2e suite and manual screenshots), and two real app-level bugs were found and fixed in the process (see Phase 4's findings). Query Suggest (D6/D7), the Semantic Encoder, and RGA are not yet started. Phase 0's email reply and presentation-slot booking are still not sent — zero-dependency, should not keep sitting idle, now three sessions running.
...
5. Create the QS model, associate to the pipeline, **enable Test Configuration Mode**. — Not started. This is why there's no search typeahead yet — `SearchBox`'s suggestion-rendering code is already complete and correct, it has nothing to render until this model exists.
6. Preload it. `PUT /rest/organizations/<ORG>/machinelearning/models/<MODEL>/configs/DEFAULT_QUERIES?languageCode=en`, multipart, form field `configFile`, UTF-8 CSV with no header, `query,weight`, max 5000 rows. Use 1025 names at weight 1 plus ~40 curated intent phrases. **Screenshot the 2xx.** This file is the documented mechanism the challenge means by "preload a Query Suggest model." — Not started.
7. Wait ~30 min for the build, then verify typeahead. — Not started.
8. Create the Semantic Encoder model on the same content. — Not started.
9. Create the RGA model and associate it to the pipeline. — Not started.
10. Tighten CSP `connect-src` to the real org hostnames in `next.config.ts`. — Not started.
