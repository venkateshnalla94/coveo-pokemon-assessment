...
**Status as of 2026-08-27 (end of the fourth session):** Phase 1's doc/CI fixes and all of Phase 2 are done. Phase 1's hosting steps (initial commit, GitHub repo, Vercel deploy) are deliberately parked — not started, pending explicit go-ahead. Phase 3 is complete (Stage A/B/C). Phase 4 is complete except step 10 (CSP tightening) — API keys, pipeline, auth wiring, Query Suggest (built, preloaded, verified live), Semantic Encoder, and RGA (built, associated, verified live with a real generated answer) are all done; Vercel env vars still pending as part of the deploy step. Phase 5 (Passage Retrieval) turned out **not to be blocked on the Phase 0 enablement email** the way this plan originally assumed — see the Phase 5 section below and `docs/adr/0008-passage-retrieval-needs-execute-query-not-content-preview.md` — the CPR model, the `/api/passages` route, and the "Ask about this Pokemon" UI are all built and verified live; only the point-of-view writeup is left. Phase 0's email reply and presentation-slot booking are still not sent — zero-dependency, should not keep sitting idle, now four sessions running. This session also added a cross-repo inspiration pass (`docs/inspiration-from-coveo-assesment.md`) mining a sibling Coveo project for reusable patterns — not part of this plan's phases, a standalone reference doc.
...
10. Tighten CSP `connect-src` to the real org hostnames in `next.config.ts`. — Not started. `docs/inspiration-from-coveo-assesment.md` item 3 (from this session's cross-repo pass) adds two more directives worth doing in the same pass: `base-uri 'self'`, `form-action 'self'`.
