...
**`Pokemon Type Effectiveness` postConversion extension (ADR-0023), separate extension object from the IPE above** — grounds weakness/resistance facts for RGA/CPR, which only ever embed `body`, never metadata. Data access = Body text only. Reads the same `pokemonweaknessesraw`/`pokemonresistancesraw` values, buckets each into weak (`super-effective`)/resist (`not very effective`)/immune (`no effect` — the one distinction `pokemonweaknesses`/`pokemonresistances` collapse together into "resistances"), and appends one plain-string sentence to `body_text` per bucket (e.g. Garchomp: weak to Ice/Dragon/Fairy, resists Fire/Poison/Rock, immune to Electric). Live on both `Pokedex - Test` and `Pokedex - Full`. Not verifiable via Content Browser's Fields JSON (never includes `body`) or Quick View (renders `body_html`, not `body_text`) — verify with a real keyword search for a phrase only this sentence produces (`"Takes extra damage from"` / `"Takes no damage from"`).
