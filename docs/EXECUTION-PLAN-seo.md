...
## Phase 2 — Structured data — deferred, not started

- [ ] `BreadcrumbList` JSON-LD on the PDP, generated from the same data `src/components/Breadcrumb.tsx` already renders — no second source of truth for the breadcrumb trail.
...
## Phase 4 — SSR body content (stretch goal, explicit go/no-go required before starting)

Not started. Confirmed live this session (curling the built PDP for pikachu) that the gap this phase would close is real: the server-rendered `<body>` briefly shows the client component's own "Pokemon not found" text before client-side hydration runs the actual Headless query — the `<head>` metadata is correct and complete, but a non-JS client would see that not-found text as the page's only real content. This is the exact, expected boundary of Phases 1–3 (metadata + status code only, per ADR-0019), not a regression introduced this session. Still flagged as optional/needs-a-go/no-go per the original reasoning below.

Phases 1–3 fix metadata/crawlability/structured-data/status-code gaps; this phase would additionally make the PDP's actual visible content (name, types, stats) present in the HTML a non-JS client receives, not just the `<head>`. That requires fetching the Pokemon server-side once and threading it into the client component as initial data (avoiding a second, redundant client-side fetch), which is a bigger and more invasive change than Phases 1–3 combined. Given this is a take-home assessment app rather than a production catalog that needs organic ranking, this is flagged as optional — needs a separate decision before any code is written, not assumed in scope by the rest of this plan.
...
- [x] `docs/adr/0019-server-rendered-seo-metadata-shim.md` — wording correction applied this session (`COVEO_API_KEY` server pattern, not `NEXT_PUBLIC_COVEO_ACCESS_TOKEN` — see that file's Context section).
- [x] `docs/HANDOFF.md` — updated this session.
